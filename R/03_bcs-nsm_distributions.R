...
#' @param mu vector of the marginal scale parameters in \eqn{(0, \infty)^d}, where \eqn{d} is the
#'     dimension.
#' @param sigma vector of the marginal relative dispersion parameters in \eqn{(0, \infty)^d}, where
#'     \eqn{d} is the dimension.
#' @param lambda vector of the marginal skewness parameters in \eqn{\mathbb{R}^d}, where \eqn{d} is the
#'     dimension.
...
#'     should be used to generate the normal scale mixture copula. Currently,
...
#' @param margins a character or a character vector; specifies the marginal Box-Cox symmetric (BCS)
#'     distributions. If all margins are the same, it is sufficient to enter only one character. A
#'     table with the current available BCS distributions can be seen in
#'     \code{\link[bcsnsm]{bcs}}.
...
#'     
#'     Because of this simple relationship between the elements of the association matrix and 
#'     Kendall's tau, we can parameterize \eqn{\bold{\Gamma}} in terms of a real-valued vector 
#'     \eqn{\bold{\gamma}} in an interpretative way depending on the type of association that is of 
#'     interest to describe. Some available parameterizations for \eqn{\gamma_{ij}}, \eqn{i \neq j}, are
#'     \describe{
#'     \item{\code{association = "nonassociative"}}{\eqn{\gamma_{ij} = 0};}
#'     \item{\code{association = "uniform"}}{\eqn{\gamma_{ij} = \gamma};}
#'     \item{\code{association = "unstructured"}}{corresponding to \eqn{\bold{\Gamma}} without any specific structure.}
#'     }
#'
#' @references
