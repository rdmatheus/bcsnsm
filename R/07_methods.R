...
#' @param labels an optional named vector used to relabel the facets in
#'     marginal diagnostic plots (i.e., when \code{type = "marginal"}). The names
#'     of \code{labels} must match the variable names of the marginal responses,
#'     and the corresponding values provide the labels to be displayed at the top
#'     of each panel. If \code{NULL} (default), the original variable names are
#'     used.
#' @param ... further arguments passed to or from other methods.
#'
...
plot.bcsnsm <- function(x, type = c("quantile", "marginal", "mahalanobis", "epsilon"),
                          levels = c(1e-1, 1e-2, 1e-3, 1e-4), panel = NULL, labels, ...) {

  dots <- list(...)
  
  col <- if (is.null(dots$col)) "#56B1F7" else dots$col
  fill <- if (is.null(dots$fill)) "#cceff1" else dots$fill
  lwd <- if (is.null(dots$lwd)) 0.5 else dots$lwd
  pch <- if (is.null(dots$pch)) "+" else dots$pch
  ...
  if (missing(labels)) labels <- colnames(y)
  names(labels) <- colnames(y)
  copula <- x$copula
  eta <- x$eta
  ...
  if (type == "quantile") {
    ...
    band <- data.frame(
      x = zi,
      lower = rq_hat - stats::qnorm(1 - 0.05 / 2) * se,
      upper = rq_hat + stats::qnorm(1 - 0.05 / 2) * se
    )

    lty <- if (is.null(dots$lty)) 1 else dots$lty
    xlab <- if (is.null(dots$xlab)) "Normal quantiles" else dots$xlab
    ylab <- if (is.null(dots$ylab)) "Residual quantiles" else dots$ylab
    cex <- if (is.null(dots$cex)) 2 else dots$cex
    
    # ggplot2::ggplot(data.frame(zi = zi, rq = rq)) +
    #   ggplot2::geom_polygon(data = positions, ggplot2::aes(x = x, y = y), fill = fill, col = col) +
    #   ggplot2::geom_point(ggplot2::aes(x = zi, y = rq), pch = pch, cex = cex) +
    #   ggplot2::xlab(xlab) + ggplot2::ylab(ylab) +
    #   ggplot2::geom_qq_line(ggplot2::aes(sample = rq), col = col, lwd = lwd, lty = lty)
    
    ggplot2::ggplot(data.frame(zi = zi, rq = rq)) +
      ggplot2::geom_ribbon(data = band, ggplot2::aes(x = x, ymin = lower, ymax = upper),
                  fill = fill, color = col, alpha = 0.3) +
      ggplot2::geom_point(ggplot2::aes(x = zi, y = rq), pch = pch, cex = cex) +
      ggplot2::geom_qq_line(ggplot2::aes(sample = rq), col = col, linewidth = lwd, linetype = lty) +
    ...
    ggplot2::ggplot(res) +
      ggplot2::geom_ribbon(ggplot2::aes(x = z, ymax = upper, ymin = lower),  data = band, fill = fill,  color = col,
                           show.legend = FALSE, lwd = lwd) +
      ggplot2::geom_point(ggplot2::aes(x = theo, y = emp), pch = pch, cex = cex) +
      ggplot2::geom_qq_line(ggplot2::aes(sample = emp), col = col, lwd = lwd, lty = lty) +
    ...
    xlab <- if (is.null(dots$xlab)) "Observation index" else dots$xlab
    ylab <- if (is.null(dots$ylab)) "Transformed Mahalanobis distances" else dots$ylab    
    ...
    diag_func <- function(data, mapping, ...){
      ...
      fill <- if (is.null(dots$fill)) "white" else dots$fill
      col <- if (is.null(dots$col)) "#00AFBB" else dots$fill
      ...
      ggplot2::ggplot(data, mapping) +
        ...
        ggplot2::geom_function(fun = dPSI, col = col)
